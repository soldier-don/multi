...
#define PAYLOAD_SIZE 20  // This now directly represents bytes
...
public:
    ...
    void generate_payload(char *buffer, size_t size) {
        for (size_t i = 0; i < size; i++) {
            buffer[i] = static_cast<char>(rand() % 256);  // Generate random bytes
        }
    }
...
// Compile with: g++ -std=c++14 soulcracks.cpp -o soul -pthread